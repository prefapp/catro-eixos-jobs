...
class DriverMongo extends Driver{
    ...
    __bucle(){
      ...
      (async () => {
          ...
          const jobs = this.buffer.getBuffer(this.store.store);

          for(let i = 0; i < jobs.length; i++){

              delete this.buffer.jobs[jobs[i]];
 ...
 //                 console.log(`en ${JSON.stringify(jobs[i], 0, 3)}`)
              ...
              if(!this.buffer.jobs[jobs[i]])
                this.buffer.jobAlmacenado(jobs[i], this.store.store);
